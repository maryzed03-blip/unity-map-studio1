  ...
  async save(mapId: string, state: CanvasState, opts?: { inline?: boolean }): Promise<void> {
    ...
    if (opts?.inline) {
      ...
      try {
        ...
        // Log AND rethrow: callers (forceSaveBoard, the debounced
        // autosave's status indicator, session-end logic) must be able to
        // tell a failed write from a successful one. Swallowing this here
        // previously meant every caller assumed "saved" even when nothing
